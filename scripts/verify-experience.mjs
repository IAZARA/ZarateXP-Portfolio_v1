...
const checks = [
  ...
  ['recruiter-first desktop order', /data-program-name="recruiter-route"[\s\S]{0,500}data-program-name="resume"[\s\S]{0,500}data-program-name="projects"[\s\S]{0,500}data-program-name="contact"/],
  ['single tap mobile launch', /touchend[\s\S]{0,500}openIcon\(icon\)/],
  ...
  ['viewport transition containment', /fitWindowsToViewport[\s\S]*availableWidth[\s\S]*availableHeight/],
  ...
  ['My Computer removable drive', /Disco extraíble \(F:\)[\s\S]*data-mipc-entry/],
  ['My Computer empty folders', /Documentos[\s\S]*Fotos varias[\s\S]*Esta carpeta está vacía/],
  ['My Computer full-height Explorer', /(?=[\s\S]*#mipc-window\s*\{[\s\S]{0,400}height:\s*100%)(?=[\s\S]*#mipc-window \.mipc-main-content\s*\{[\s\S]{0,400}flex:\s*1)/],
  ['safe removable-drive ejection', /ejectRemovableDrive[\s\S]*zaratexp:removable-drive-ejected[\s\S]*Es seguro quitar el hardware/],
  ...
  ['current professional headline', /AI Solution Architect en ACSYS \| Software, Data &amp; AI Solutions/],
  ['honest current role schema', /"jobTitle":\s*"AI Solution Architect"/],
  ['current employer schema', /"worksFor":\s*\{[\s\S]*?"name":\s*"ACSYS"/],
  ['ACSYS role positioning', /AI Solution Architect en ACSYS[\s\S]*discovery[\s\S]*arquitectura de IA/i],
  ['institutional case evidence', /CUFRE[\s\S]*SIFEBU[\s\S]*CRIACO[\s\S]*OSINTArgy/],
  ['professional experience timeline', /2026 - Actualidad[\s\S]*2024 - 2026[\s\S]*2018 - 2024[\s\S]*2016 - 2018[\s\S]*2013 - 2015/],
  ['official ACSYS logo', /assets\/images\/acsys-logo\.svg[\s\S]*data-acsys-logo="official"/],
  ['education and languages', /Google Data Analytics[\s\S]*Inglés/],
  ['certificates explorer', /data-certificates-root[\s\S]*data-certificate-filter[\s\S]*data-certificate-preview/],
  ['sixteen verified credentials', /Claude Code 101[\s\S]*AI Capabilities and Limitations[\s\S]*Google AI Professional Certificate[\s\S]*Google Data Analytics Professional Certificate[\s\S]*Fundamentos de la gestión de proyectos[\s\S]*ArcGIS Pro Flujos de Trabajo[\s\S]*Operador de Drones en ArcGIS/],
  ['Claude Academy public verification links', /e759d3c0-b384-4295-ae87-8dd66724db6f[\s\S]*a410f5a6-bede-48ad-9128-720a1f6802a3/],
  ['certificate highlights', /data-certificate-featured[\s\S]*data-certificate-filter="featured"/],
  ['Coursera public verification links', /DYU79Z46B5D0[\s\S]*T29Z4TO57M3H[\s\S]*WHLKPKLWLRWJ/],
  ['certificate responsive explorer', /xp-certificates-layout[\s\S]*@media \(max-width: 760px\)[\s\S]*xp-certificate-list[\s\S]*scroll-snap-type/],
  ...
  ['project highlights', /featured:\s*\['zaratexp',\s*'osintargy',\s*'forzatask',\s*'arana-web',\s*'auto-inbox',\s*'art-redmine',\s*'workflow-black-box',\s*'cufre'\]/],
  ...
  ['skippable boot', /boot-skip[\s\S]*waitForBootOrSkip/],
  ['lazy EmailJS', /_loadScriptOnce\('assets\/vendor\/email\.min\.js',\s*'emailjs'\)/],
...
const forbidden = [
  ...
  ['outdated FDE opportunity positioning', /orientad[oa] a oportunidades?(?: de)? Forward Deployed Engineer|Profile oriented to Forward Deployed Engineer opportunities/i],
